...
export class PowerFailuresDailyEffects {
    ...
    loadPowerMonitorDailyData$ = createEffect(() =>
        this.actions$.pipe(
            ...
            mergeMap(({ date }) =>
                this.powerService.getPowerFailuresDailyData(date.getFullYear(), date.getMonth() + 1).pipe(
                    mergeMap((data) => {
                        const newState = {} as FailuresDailyState;
                        newState.data = data;
                        newState.date = date;
                        const maxPowerFailure = data
                            .find(o => o.duration === Math.max.apply(null, data.map(e => e.duration)));
                        if (maxPowerFailure) {
