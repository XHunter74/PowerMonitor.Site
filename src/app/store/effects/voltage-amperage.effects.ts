...
} from '../actions/voltage-amperage.actions';
            ...
            mergeMap(({ date }) =>
                this.powerService.getVoltageAmperageData(date).pipe(
                    map((data) => {
                        ...
                        newState.data = data;
                        newState.date = date;
                        newState.maxVoltage = data.find(
                            (o) => o.voltageMax === Math.max(...data.map((e) => e.voltageMax)),
                        );
                        newState.minVoltage = data.find(
                            (o) => o.voltageMin === Math.min(...data.map((e) => e.voltageMin)),
                        );
                        newState.maxAmperage = data.find(
                            (o) => o.amperageMax === Math.max(...data.map((e) => e.amperageMax)),
                        );
                        newState.minAmperage = data.find(
                            (o) => o.amperageMin === Math.min(...data.map((e) => e.amperageMin)),
                        );
