...
import { loadMonthlyFailuresData, loadMonthlyFailuresDataSuccess, loadMonthlyFailuresDataFailure } from '../actions/power-failures.monthly.actions';
...
export class PowerFailuresMonthlyEffects {
    ...
    loadPowerMonitorMonthlyData$ = createEffect(() =>
        this.actions$.pipe(
            ...
            mergeMap(({ date }) =>
                this.powerService.getPowerFailuresMonthlyData(date.getFullYear())
                    .pipe(
                        mergeMap((data) => {
                            return from(data).pipe(
                                ...
                                mergeMap((updatedData) => {
                                    ...
                                    newState.date = date;
                                    newState.totalPowerFailure = updatedData.reduce((a, b) => a + b.duration, 0);
                                    newState.failureAmount = updatedData.reduce((a, b) => a + b.events, 0);
    ...
    formatMonth(date: Date) {
        return this.translate.get(`MONTHS.M${date.getMonth()}`).pipe(
            mergeMap((month) => of(`${month} ${date.getFullYear()}`))
        );
    }
}