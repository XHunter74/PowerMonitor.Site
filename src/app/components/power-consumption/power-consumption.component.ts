...
} from '../../store/reducers/power-consumption.reducer';
...
} from '../../store/actions/power-consumption.actions';
...
export class PowerConsumptionComponent extends AppBaseComponent implements OnInit, OnDestroy {
    ...
    async ngOnInit() {
        ...
        this.deleteStateSubscription = this.powerConsumptionDeleteState$.subscribe((state) => {
            if (
                state &&
                (state.loading || state.error || (!state.loading && state.operationComplete))
            ) {
    ...
    deleteRecord(recordId: number) {
        this.translate
            .get(['POWER_CONSUMPTION.DELETE_QUESTION', 'COMMON.YES', 'COMMON.NO'])
            .subscribe((translations) => {
                ...
                QuestionDialogComponent.show(this.dialog, questionText, yesText, noText).then(
                    (dialogResult) => {
